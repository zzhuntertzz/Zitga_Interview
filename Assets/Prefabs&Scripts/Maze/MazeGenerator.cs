...
{
    // public static List<Position> Paths = new List<Position>();
    public static Position startPos, endPos;
    ...
    private static WallState[,] ApplyRecursiveBacktracker(WallState[,] maze, int width, int height, int seed)
    {
        ...
        var position = new Position { X = 0, Y = height - 1 };
        ...
        while (positionStack.Count > 0)
        {
            ...
            // {
            //     endPath = true;
            //     Debug.Log($"End: {current.X} {current.Y}");
            // }
        }

        rng = new System.Random(); //removeSeed
        endPos = startPos;
        while (Mathf.Abs(endPos.X - startPos.X) < 3 && Mathf.Abs(endPos.Y - startPos.Y) < 3)
        {
            endPos = new Position { X = rng.Next(0, width), Y = rng.Next(0, height) };
        }
        return maze;
    }
